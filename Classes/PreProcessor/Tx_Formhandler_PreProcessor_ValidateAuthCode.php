...
class Tx_Formhandler_PreProcessor_ValidateAuthCode extends Tx_Formhandler_AbstractPreProcessor {
	...
	public function process() {
		if($this->gp['authCode']) {
			...
			try {
				...
				$uidField = $this->wrapForSQLQuery($uidField);
				$wrappedTable = $this->wrapForSQLQuery($table);
				...
				$res = $GLOBALS['TYPO3_DB']->exec_UPDATEquery($wrappedTable, $uidField . '=' . $uid, array($hiddenField => 0));
				if(!$res) {
	...
	protected function wrapForSQLQuery($str) {
		$str = '`' . $str . '`';
		return $str;
	}
